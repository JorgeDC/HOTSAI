...
hots_results = np.array(hots_all[:,features_count])
hots_features = np.array(hots_all[:,:features_count])

print(hots_results.shape)

num_rows, num_cols = hots_features.shape
...
with tf.Session() as session:
    ...
    for epoch_i in range(epochs):
         ...
         for x, y in getBatches(train_x, train_y, batch_size):
            ...
            loss, state, _ = session.run([cost, final_state, optimizer], feed_dict={features: x, labels: y[:,None], keep_prob:dropout_keep_prob})
            #print(loss)
            ...
            if iteration % 1000 == 0:
                ...
                for x_val, y_val in getBatches(val_x, val_y, batch_size):
                    #print(x_val.shape)
                    #print(y_val.shape)
                    feed = {features: x_val, labels: y_val[:,None], keep_prob: 1.0, initial_state:val_state}
