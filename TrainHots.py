...
data_all = 'all_results.csv'

hots_all = np.array(pd.read_csv(data_all))

hots_results = np.array(hots_all[:,[140, 141]])
hots_features = np.array(hots_all[:,:140])

num_rows, num_cols = hots_results.shape

train_val_set_count = math.floor((num_rows / 10) * 9)

print(hots_features.shape)
...
net = tflearn.input_data([None, 140])
